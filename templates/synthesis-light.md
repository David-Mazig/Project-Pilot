# Session Synthesis Instructions — LIGHT

> **This is the LIGHT weight synthesis file.** Use for sessions with fewer than 6 changes.
> For 6-15 changes use synthesis-medium.md. For 16+ or every 5th session use synthesis-heavy.md.
...
3. Update .pilot/progress.md — categorize features accurately using test results if available:
   ...
   **Never assume a feature works just because code was written.** Default to "Implemented (Unverified)" unless `.test-results` says PASS.
   ...
   After updating progress.md, delete `.test-results` so it does not carry over to the next session:
   ```bash
